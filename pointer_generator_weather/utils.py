...
word_detector = re.compile(u"[\u4e00-\u9fa5。，！？]+")
...
class Vocab(object):
    ...
    def trim(self, *, vocab_size: int = None, min_freq: int = 2): # 注意min_freq的设置很有可能会将vocab大小变得很小
...
class Dataset(object):
    ...
    def build_vocab(self, vocab_size: int = None, src: bool = True, tgt: bool = True,
        ...
        if os.path.isfile(filename):
            ...
            # # 输出一下vocab的词频统计
            # import codecs
            # with codecs.open('../data/chinese/lan/weather_vocab.json', 'w', 'utf-8') as f:
            #     json.dump(vocab.word2count, f, ensure_ascii=False)
            # print("Vocabulary loaded, %d words." % len(vocab))
            return vocab # 如果已经有词典文件，就不加载原来的预训练文件了
        else:
        ...
        for hyp, ref in zip(preds, target):
            ...
            for word in ref:
                if word not in w2id:
                    w2id[word] = p
                    p += 1
            hyp = [str(w2id[word]) for word in hyp]
            ref = [str(w2id[word]) for word in ref]
