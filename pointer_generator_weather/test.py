...
from pointer_generator_weather.utils import rouge, Vocab, OOVDict, Batch, format_tokens, format_rouge_scores, Dataset
from pointer_generator_weather.model import DEVICE, Seq2SeqOutput, Seq2Seq
...
def eval_batch_output(tgt_tensor_or_tokens: Union[torch.Tensor, List[List[str]]], vocab: Vocab,
    ...
    :param pred_tensors: one or more systems' prediction (output tensors) (一个或两个decode_batch(可能一个是瞎几把生成的，
                                                                           一个是贪心生成的))
...
if __name__ == "__main__":
    ...
    if args.model:  # evaluate a specific model
        ...
        filename = '%s_%02d.pt' % (p.model_path_prefix, train_status['best_epoch_so_far'])
