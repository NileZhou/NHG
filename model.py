...
from utils2 import Vocab, Hypothesis
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, embedded, hidden, src_lens=None):
        """
        :param embedded: (src seq len, batch size, embed size)
        :param hidden: (num directions, batch size, encoder hidden size)
        :param src_lens: list containing the non-padded length of each sequence in this batch;
        ...
        if src_lens is not None:
            embedded = pack_padded_sequence(embedded, src_lens)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, embedded, hidden, encoder_states=None, decoder_states=None, coverage_vector=None, *,
        ...
        if self.pointer:
            ...
            prob_ptr = torch.sigmoid(self.ptr(combined)) # 上面那句被deprecated了
            prob_gen = 1 - prob_ptr
