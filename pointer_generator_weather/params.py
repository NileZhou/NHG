...
class Params:
    # Model architecture
    vocab_size: int = 10000 # 小数据集，10000大小的词表足矣
    hidden_size: int = 512  # of the encoder; default decoder size is doubled if encoder is bidi
    dec_hidden_size: Optional[int] = 256  # if set, a matrix will transform enc state into dec state
    embed_size: int = 100
    enc_bidi: bool = True
    ...
    cover_func: str = 'sum'  # how to aggregate previous attention distributions? sum or max
    cover_loss: float = 2  # add coverage loss if > 0; weight of coverage loss as compared to NLLLoss
    show_cover_loss: bool = True  # include coverage loss in the loss shown in the progress bar?

    # Regularization
    enc_rnn_dropout: float = 0.5 # 如果过拟合，全部改为0.5
    dec_in_dropout: float = 0.5
    dec_rnn_dropout: float = 0.5
    dec_out_dropout: float = 0.5
    ...
    lr_decay: Optional[float] = 0.9  # decay lr by multiplying this factor
    batch_size: int = 16
    ...
    forcing_ratio: float = 0.25  # 0.75 initial percentage of using teacher forcing
    ...
    # Data
    embed_file: Optional[str] = '../data/chinese/sum2tit/embedding.txt'  # use pre-trained embeddings
    data_path: str = '../data/chinese/sum2tit/train.txt'
    val_data_path: Optional[str] = '../data/chinese/sum2tit/val.txt'
    max_src_len: int = 70  # (准确说是61) exclusive of special tokens such as EOS
    max_tgt_len: int = 20  # (准确说是11) exclusive of special tokens such as EOS
    ...
    model_path_prefix: Optional[str] = 'checkpoints/pointer_generator'
    ...
    # 标题word在3-15之间
    min_out_len: int = 3
    max_out_len: Optional[int] = 20
    out_len_in_words: bool = False
    test_data_path: str = '../data/chinese/sum2tit/test.txt'
