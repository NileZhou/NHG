...
# model
parser.add_argument('-save_dir', type=str, default='little/checkpoints/')
...
parser.add_argument('-val_dir', type=str, default='../data/chinese/cont2sum/little/val.json')
...
# test
parser.add_argument('-load_dir', type=str, default='little/checkpoints/AttnRNN_seed_1.pt')
...
def train():
    ...
    train_dataset = summarunner_weather.utils.Dataset(examples)

    with open(args.val_dir) as f:
        examples = [json.loads(line) for line in f]
    val_dataset = summarunner_weather.utils.Dataset(examples)
...
def tst():
    ...
    with open(args.test_dir) as f:
        examples = [json.loads(line) for line in f]
    test_dataset = summarunner_weather.utils.Dataset(examples)
    ...
    for batch in tqdm(test_iter):
        ...
        for doc_id, doc_len in enumerate(doc_lens):
            ...
            topk_indices.sort()
            doc = batch['content'][doc_id].split('\n')[:doc_len]
            ...
            #     f.write('\n'.join(hyp))
            hyp = '\n'.join(hyp)
            hyp_i = [str(vocab.w2i(word)) for word in hyp.split()]
            ref_i = [str(vocab.w2i(word)) for word in ref.split()]
            res = evaluator.get_scores(' '.join(hyp_i), ' '.join(ref_i))
