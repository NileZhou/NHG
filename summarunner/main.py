...
parser.add_argument('-train_dir', type=str, default='../data/english/cont2sum/train.json')
parser.add_argument('-val_dir', type=str, default='../data/english/cont2sum/val.json')
parser.add_argument('-embedding', type=str, default='../data/english/cont2sum/embedding.npz')
parser.add_argument('-word2id', type=str, default='../data/english/cont2sum/word2id.json')
...
parser.add_argument('-test_dir', type=str, default='../data/english/cont2sum/test.json')
parser.add_argument('-ref', type=str, default='outputs2/ref')
parser.add_argument('-hyp', type=str, default='outputs2/hyp')
...
def train():
    ...
    train_dataset = summarunner.utils.Dataset(examples)

    with open(args.val_dir) as f:
        examples = [json.loads(line) for line in f]
    val_dataset = summarunner.utils.Dataset(examples)
    ...
    # build model
    net = getattr(summarunner.models, args.model)(args, embed)
    if use_gpu:
        net.cuda()
    # load dataset
...
def tst():
    ...
    with open(args.test_dir) as f:
        examples = [json.loads(line) for line in f]
    test_dataset = summarunner.utils.Dataset(examples)
    ...
    print(df.describe())
    print('Speed: %.2f docs / res' % (doc_num / time_cost))
