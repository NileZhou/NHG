...
def train():
    ...
    train_dataset = summarunner_can_run.utils.Dataset(examples)

    with open(args.val_dir) as f:
        examples = [json.loads(line) for line in f]
    val_dataset = summarunner_can_run.utils.Dataset(examples)
    ...
    # build model
    net = getattr(summarunner_can_run.models, args.model)(args, embed)
    if use_gpu:
        net.cuda()
    # load dataset
...
def test():
    ...
    with open(args.test_dir) as f:
        examples = [json.loads(line) for line in f]
    test_dataset = summarunner_can_run.utils.Dataset(examples)
