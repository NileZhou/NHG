...
from pointer_generator_weather.utils import Dataset, show_plot, Vocab, Batch
from pointer_generator_weather.model import Seq2Seq, DEVICE
from pointer_generator_weather.params import Params
from pointer_generator_weather.test import eval_batch, eval_batch_output
...
def train_batch(batch: Batch, model: Seq2Seq, criterion, optimizer, *,
    ...
    if rl_ratio > 0:
        ...
        # sample_out没有NLLLoss, 而且是随机采样, sample=True, 代表模型返回的"decoded_tokens"将基于随机采样(瞎几把生成)而非贪心选择最高概率的词
        ...
        greedy_rouge = scores[1]['l_f'] # 这个是greedy那个batch的rouge-f得分
...
def train(train_generator, vocab: Vocab, model: Seq2Seq, params: Params, valid_generator=None,
    ...
    for epoch_count in range(1 + past_epochs, params.n_epochs + 1):
        ...
        prog_bar = tqdm(range(1, params.n_batches + 1), desc='Epoch %02d' % epoch_count)
        model.train()
        ...
        if valid_generator is not None:  # validation batches
            ...
            plot_val_metrics.append(valid_avg_metric * 100)
        ...
        if params.model_path_prefix:
            # save model
            filename = '%s_%02d.pt' % (params.model_path_prefix, epoch_count)
            torch.save(model.state_dict(), filename) # 只保存模型参数
            if not params.keep_every_epoch:  # clear previously saved models
                for epoch_id in range(1 + past_epochs, epoch_count):
                    if epoch_id != best_epoch_id:
                        try:
                            prev_filename = '%s_%02d.pt' % (params.model_path_prefix, epoch_id)
            ...
            torch.save({
                ...
                'params': params,
                'optimizer': optimizer
            }, '%s_train.pt' % params.model_path_prefix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_from', type=str, metavar='R',
                        help='path to a saved training status (*_train.pt)')
    args, unknown_args = parser.parse_known_args()

    if args.resume_from:
        print("Resuming from %s..." % args.resume_from)
        train_status = torch.load(args.resume_from)
        m = torch.load('%s_%02d.pt' % (args.resume_from[:-9], train_status['epoch']))
